...
sources. The loaders are capable to process and ingest the three most popular
\gls{VCS} currently in use: Git, Mercurial and SVN\@. They also handle the
packages of the Linux distributions Debian, Nix and Guix, as well as those from
the repositories PyPI, CRAN, NPM and Packagist (respectively for the Python, R,
...
156 million software origins. In the \gls{VCS} history, we identified as much
...
on-disk size to around half of that.\TODO{more precise number?} The main
...
The archive is of course expected to grow over time as more and more listers,
