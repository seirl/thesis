...
code files. Developers tend to organize their code in several logical units
that are then hierarchized in multiple levels of directories. At the lowest
...
repositories, and our data model should accommodate for that possibility.
...
revisions significantly reduces the memory impact of the model, each version
...
\figref{fig:deduplicate-subtree} shows this subtree deduplication in action. In
...
artifacts will be attributed an intrinsic hash recursively computed from the
...
is not the case if a software artifact is referenced by two other artifacts. We
thus call this structure a \textbf{Merkle DAG}. Note that it, by definition,
cannot contain any cycles: because adding a node requires the hash of all its
children be already known to compute its own hash, a cycle would require
computing two mutually dependent cryptographic hashes, which is at least as
hard as finding a collision.
...
As shown in \figref{fig:okasaki-complexity}, creating a new revision with a
