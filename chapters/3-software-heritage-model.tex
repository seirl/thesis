...
object types proper to each specific system can be mapped to abstract concepts.
Git ``commits'', SVN ``revisions'' and Mercurial ``changesets'' all correspond
to the same abstract concept of a frozen state of the source tree, and thus
they are all \emph{canonicalized} as a single type of artifact that we call
``revisions''.
...
stored in canonical form, we can provide an uniform interface for researchers
to study artifacts coming from all kinds of different sources. This isolates
...
code files. They only represent the actual binary data contained in the files;
file \emph{names} are external and directory-dependent, and are not included in
this type of artifact. They are identified by an intrinsic hash of the full
binary data they contain.
...
(``revision entries''). Each entry is associated to a local binary name (i.e.,
a relative path without any path separator) and permission metadata.
Revision entries are used to encode directories referencing a specific
revision, like for Git submodules and Mercurial subrepositories.
The permission integer can encode arbitrary permissions such as whether the
file is an executable, a symbolic link, who can read or write in it, etc.
They are identified by an intrinsic hash of the textual manifest of all their
entries and their own identifiers.
...
be issued from the ``main'' branch that the revision gets merged onto, and thus
...
directory of the source tree they refer to.
...
case of a software archive however, we need to keep track of the successive
state of the repositories for each crawling \emph{visit}, which includes
retaining old information about which branches and tags used to be present in
the repository, and what they were pointing to.
...
objects, but they can also rarely point to directories and contents, which is
supported by some \glspl{VCS} and also present in some rare cases in real-world
repositories.
...
which source code artifacts have been retrieved to be ingested in the archive.
They are represented by a canonical URL (e.g., the address at which one can
\texttt{git clone} a repository or \texttt{wget} a source code tarball.)

Origins represent specific places of archival, and are distinct from the more
abstract notion of \emph{projects}: a project is an entity that can relate
together different development resources, including websites, issue trackers,
mailing lists, and software origins. A software project can migrate its
development from one origin to another for various reasons, and even migrate to
a different \gls{VCS}. Projects are ontologically complex notions that are not
directly present in the archive; the data model is mostly concerned about
directly addressable and concrete artifacts, and \emph{origins} are better
...
already allows us to canonicalize and deduplicate objects inside a single
...
This process of systematically gathering and storing all the publicly available
...
the derivative works and shared codebases.
...
In short, this means that whenever a directory, a file or any artifact is
referenced by multiple projects, it will get deduplicated as a single node in
the graph, and will be directly referenced by all the projects and artifacts
...
Merging all the archived repositories in a single large collection of software
artifacts leverages all the benefits of Merkle \gls{DAG} models for single
repository to the entire archive: simple identification of unique artifacts,
...
and analyze code reuse: walking back the \gls{DAG} can tell us the list of all
the projects that contain a specific piece of code. By looking at the commit
chains in the graph, it is possible to find out when any given file was copied,
and all the changes that were subsequently made to it.
