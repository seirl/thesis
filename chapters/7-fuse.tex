...
Provided a reference to the desired code can be obtained, the corresponding
file, directory, or commit can be used as if it were locally available.
\SWHFS{} offers a convenient way to explore huge code bases~\cite{msscalar,
msvfsforgit} that require significant time to be fully retrieved over the
network, large amounts of disk space, and high I/O costs when switching
branches.
...
Other VCS filesystems have been proposed in the past.
...
reduce disk and bandwidth usage for huge code bases, retaining compatibility
...
archive, giving access to hundreds of million code repositories in a unified
view.
...
\paragraph{Front-end: POSIX filesystem and FUSE}
...
approach avoids duplications in the virtual filesystem, reifying on disk the
sharing of Merkle structures.
...
\paragraph{Backend: a \SWH{} $\leftrightarrow$ FUSE adapter}
...
return it to the kernel via the FUSE API\@. For example, when a source tree
object is listed using \mintinline{c}{readdir()}, \SWHFS{} invokes the
\texttt{/directory} endpoint of the \SWH{} API and return to the kernel its
directory entries using a lazy iterator; when a source code file is
\mintinline{c}{open()}-ed, \texttt{/content/raw} is called and byte
chunks of the retrieved blob are returned to the kernel piecemeal.
...
Remote filesystems can be notoriously painful to use, due to network overhead.
To make things worse for \SWHFS, the backend technology stack (REST APIs and
long-term archival storage) was not designed with filesystem-level performances
in mind. In order to make it fast enough for practical use, \SWHFS{} implements
...
The \emph{blob cache} stores raw source code file contents. The \emph{metadata
  cache} stores the metadata of any kind of looked up object. Using these two
  caches it is possible to navigate any part of the \SWH{} archive that has been
accessed in the past, even while disconnected from the network.

Merkle properties and the fact that \SWHFS{} is read-only make cache invalidation
unneeded, because no cached object can ever change. It is also always possible
to purge objects from these caches without introducing inconsistencies.

\emph{In-memory caching} is used for directories, as most filesystems do. The
\emph{direntry cache} maps directories to their entries, so that frequently
accessed directories can be listed without even incurring SQLite query costs.
...
to the Vault which always bundles all the objects even when they are not
necessary for the analysis. This makes \SWHFS{} an efficient way to run
experiments that only access a small number of specific files in a repository,
e.g., license or README files.
...
issue makes \SWHFS{} often unsuitable for large-scale mining is the lack of
entry-points to find the relevant artifacts. While the virtual filesystem makes
it easy to find, for instance, all the Python files in a single repository,
there is no easy way to find all the files or revisions matching a certain
...
The next logical step is to explore ways to efficiently select massive amounts
of software artifacts according to more fine-grained criteria, and run analyses
for larger workloads, up to the size of the entire archive.
