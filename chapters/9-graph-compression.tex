...
As size benchmark, we show that the resulting compressed VCS graph, containing
...
As dataset we use the 2018-09-25 version of the Software Heritage graph dataset
...
As sole exception we will also keep in memory \emph{node types}, i.e., whether
...
compression pipeline shown in \figref{fig:compression-pipeline}. The
...
\figref{fig:compression-indegree}) has a fatter tail than the outdegree
distribution, (\figref{fig:compression-outdegree}) resulting in nodes with very
...
\figref{fig:llp-compression-pipeline} shows the full compression pipeline
...
\TODO{sentence explaining how good the results are once we have the up to date
numbers}.
...
Table~\ref{tab:compression-arc-benchmark} shows the results of benchmarking
...
\figref{fig:compression-distrib-cnt-rev} shows the multiplication factor
...
\figref{fig:compression-distrib-rev-ori} shows analogous results for the
...
The cumulative distribution of \figref{fig:compression-distrib-rev-ori} measure
...
\begin{figure}
  ...
  \caption{Visit size, as the number of visited nodes, for measuring
    ...
    \figref{fig:compression-distrib-cnt-rev}.}%
...
\begin{figure}
  ...
  \caption{Visit size, as the number of visited nodes, for measuring
    ...
    \figref{fig:compression-distrib-rev-ori}.}%
...
\figref{fig:compression-size-cnt-rev} and~\ref{fig:compression-size-rev-ori}
...
\TODO{Reference to next chapter here.}
